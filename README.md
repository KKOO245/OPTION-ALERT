...
已知限制，不是bug)，所以工作流改成了**整点触发、间隔1小时**这种GitHub认为
可靠的频率：每天在UTC 12-16点和18-22点各整点跑一次(共10次)，在每个目标时间
前后安排了2-3次独立的备份尝试。脚本自己判断现在是不是接近10:30或16:30，
命中了才会真的抓数据发邮件，没命中就几十秒内直接结束。同一天同一个时段只会
真正发送一次(防止好几次整点尝试都命中同一个时段导致重复发送)。
...
那边的cron窗口通常不需要跟着改(除非新时间点已经超出UTC 12-16点/18-22点覆盖的
多伦多时间早7点-晚8点这个范围)。

### 关于GitHub Actions免费分钟数会不会超

个人账户免费额度是每月2000分钟(私有仓库；公开仓库不限)。现在每天只有10次
整点尝试(比之前"每15分钟检查一次"的方案少了大约80%)，每次运行几十秒到1分钟
左右，每月消耗大概在150-250分钟左右，完全不用担心超额度。想亲自核实的话，
去仓库 `Settings` → 左边 `Billing and plans`(或GitHub账户设置里的 `Billing`)
能看到"Actions"这一项的实际分钟消耗。

> 注意：GitHub Actions的定时任务不保证100%准点，脚本里留了50分钟的容差
> (`TOLERANCE_MINUTES`)，配合每小时一次的整点尝试，正常延迟不会导致漏发。
