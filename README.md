...
- **早报（多伦多 10:15，开盘后约 45 分钟）**：开盘快照、异动、今日关注
...
**所有涉及标的（ticker）的改动——新增、删除、暂停、恢复、调整监控范围——一律只修改 `config/tickers.txt` 这一个文件**，不碰代码、不碰其他配置。

- `config/tickers.txt` 是标的范围的**唯一事实来源**：任何功能、报告、测试、工作流都从它读取标的清单，禁止在别处另建一份标的列表。
- 以后任何改动或更新（代码、报告、配置、工作流、文档）都必须先检查该文件是否需要同步，涉及标的的改动一律落在这个文件里。
- 标的数量上限 **20 个**（超限会打印警告并拒绝继续）。
...
2. **按未平仓量 Top5（到期日维度）**：最近到期日的看涨/看跌 Top5 + 未来 4 个期权日合并的看涨/看跌 Top5（含行权价、最新价、IV）
3. **异动成交 Top（含前后对比）**：成交量/OI 异常放大的合约（带到期日），附"昨量 vs 今量、放量倍数、OI 前值 vs 现值、OI 增减%"
4. **OI 增仓 Top**：未平仓量较上次快照明显增加的合约（疑似新建仓）
5. **OI 集中带**：现价附近未平仓堆叠最重的行权价（支撑/压力参考）
6. **规则型解读**：每个指标对应的方向性/风险文字（不依赖任何外部服务，永远可用）
7. **AI 深度分析**（可选，推荐开启）：把当天指标交给 AI 模型，生成市场背景、逐标的要点、风险提示、明日关注四节专业中文分析；AI 只解读算好的数字、不自己算数，失败时自动退回规则版
8. **数据附录**：每只标的的核心数字一行列全，方便你核对
9. **日历（仅周二/周四早报）**：本周美国高/中重要性宏观数据（美联储会议、CPI/PPI、非农、失业率等，含预测值/实际值/前值）+ 当周重要公司财报（白名单见 `config/earnings_watchlist.txt`）
10. **市场情绪（早报）**：CNN 恐惧贪婪指数（当天分值 + 昨收，来源 CNN 官方公开接口）
11. **市场结构深度解读（每标的）**：Call/Put Wall、Gamma Flip（零 Gamma）、Net GEX、Dealer Gamma、Top Gamma 行权价、Net Vanna/Charm、成交量集中行权价，并生成"上方压力位 / 下方支撑位 / Gamma 区域 / 做市商对冲方式 / 短线偏多偏空"五段式解读
...
- 兜底数据源：**Yahoo Finance**
- 宏观日历：TradingView 经济日历公开接口；恐惧贪婪指数：CNN 官方公开接口
...
只需要编辑 `config/tickers.txt`：每行一个代码，`#` 开头表示注释。**铁律：涉及标的的任何改动都只动这一个文件，不需要改任何代码。**

---

## 关于运行时间与可靠性

- 目标时间：多伦多 **10:15 / 16:30**，周一至周五；夏令时/冬令时自动适配，全年不用手动改
...
OPTION-ALERT/
├── config/
│   ├── tickers.txt          ← 监控标的（你唯一需要经常编辑的文件）
│   └── earnings_watchlist.txt ← 财报日历的白名单（可自行增删公司）
├── src/
...
**Q: 会占用很多 GitHub Actions 免费分钟数吗？** 不会。系统每天会触发约 13 次（每小时一次做防漏发兜底），但工作流里加了"快速时间检查"：非目标时段的空跑在装依赖之前就结束，每次只花约半分钟；真正抓数据、算指标、发报告的重活每天只有 2 次、每次约 2-4 分钟。每月合计约 **150-250 分钟**，远低于免费额度 2,000 分钟（私有仓库）。
