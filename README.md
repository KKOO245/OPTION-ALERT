...
默认是多伦多时间每天 **10:30** 和 **16:30**，只在周一到周五。因为GitHub免费额度
的定时任务实际触发时间可能延迟几十分钟到几个小时，这里没有用"固定时间点硬等"的
办法，而是让工作流**每15分钟检查一次**(只在UTC 12:00-24:00这个窗口内检查，
覆盖了目标时间前后好几小时的缓冲)，脚本自己判断现在是不是接近10:30或16:30，
命中了才会真的抓数据发邮件，没命中就几十秒内直接结束，几乎不消耗资源。
同一天同一个时段只会真正发送一次(防止15分钟检查一次导致连续发好几封重复邮件)。
...
那边的cron窗口通常不需要跟着改(除非新时间点已经超出UTC 12:00-24:00覆盖的
多伦多时间早7点-晚8点这个范围)。

### 关于GitHub Actions免费分钟数会不会超

个人账户免费额度是每月2000分钟(私有仓库；公开仓库不限)。现在的检查频率下，
每天大约48次运行，绝大多数是"跳过型"的空转(几十秒结束)，只有2次左右是真正
抓数据、发邮件的完整运行，粗略估算下来每月消耗在700-800分钟左右，还有不少余量。
想亲自核实的话，去仓库 `Settings` → 左边 `Billing and plans`
(或GitHub账户设置里的 `Billing`) 能看到"Actions"这一项的实际分钟消耗。

> 注意：GitHub Actions的定时任务不保证100%准点，脚本里留了20分钟的容差
> (`TOLERANCE_MINUTES`)，配合15分钟一次的检查频率，正常延迟不会导致漏发。
