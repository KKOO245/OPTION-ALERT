...
Options: P/C成交量 0.97 | OI比 1.07 | ATM IV 11.0% | Skew 0.9pp | Term 1.12 | ExpMove ±0.9%（近端） | Rank 32%
量化视角： IV 中性（Rank 32%）｜期限结构正常（Term 1.12）｜保护溢价薄（Skew 0.9pp）｜当日成交与存量接近均衡——观察点，非方向信号
   ⇒ Put/Call Volume: 0.97×（Put 与 Call 成交量接近）→ 方向 Unknown
   ⇒ Put/Call OI: 1.07×（两侧接近均衡）
   ⇒ 当日成交 vs 存量仓位：当日成交接近均衡，存量接近均衡
   ⇒ 历史分位（15年 lambdaclass 全链口径）: GEX 53% ｜ P/C OI(近端) 3%
量化视角的组合解读： Gamma 处于历史中位（GEX 分位 53%）｜近端持仓极端 Call 重（P/C OI 分位 3%，历史极低区）——观察点，非方向信号
   ExpMove 期限化（expmove_v1）: 09-24（1D）±0.9% ｜ 09-25（2D）±1.1% ｜ 09-28（5D）±1.2% ｜ 09-29（6D）±1.2%
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) -114,835,499 | GEX Change vs 上次快照 -813,919,565 | Flip: Primary Flip: 768.50（PRIMARY，全链重定价 + 覆盖达标）
...
结构观察区: Primary Flip 768.50（全链重定价，覆盖 100%）
Call Wall 785（弱结构｜现价低于该位 2.2%）
最近结构参考: Flip 768（现价低于该位 0.1%）
量化视角： 负 Gamma（1.15亿，历史分位 53%，中性区）｜由正转负（8.14亿）｜现价位于 Flip 下方 0.09%｜⚠️ 重点观察：负 Gamma 且日内加深——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
量化解读： 存量两侧均衡｜ATM IV 11.0%｜历史 Rank 32%（近端代理）｜IV/RV 1.07×（近似）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 0 股
