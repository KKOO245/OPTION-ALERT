...
TSLA: 今晨 354.71 → 收盘 348.08（-1.9%）
Target 状态: PENDING（evaluation date …）——窗口结束前禁止'预测正确'类措辞


## TSLA

Options: P/C量 0.86 | OI比 0.89 | ATM IV 49.8% | Skew 0.0pp | Term 0.81 | ExpMove ±0.5% | Rank — (历史不足)
   ⇒ Put/Call Volume: 0.86×（Call 成交高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.89×（Put OI 低于 Call OI）→ 存量 Call-dominant
   ⇒ 两者结构一致
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma: POSITIVE | GEX(存量) N/A | GEX Change N/A | Flip: ≈354.15 / ≈366.89 / ≈387.39
结构观察区: 354–367（局部 Gamma 切换，低置信；Top-3 近似，需全链重定价验证）
距 Put Wall 300: +16.0% | 距 Call Wall 400: -13.0%
🧭 结构解读（全部依赖上方假设）
• 支撑/压力参考：下方 300（Put Wall）；上方 400（Call Wall）。
• Gamma 区域：切换参考 354（Top-3 近似，需全链重定价验证）。
