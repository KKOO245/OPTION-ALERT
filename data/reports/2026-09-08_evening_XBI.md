...
Options: P/C成交量 1.22 | OI比 1.80 | ATM IV 35.7% | Skew 0.2pp | Term 0.85 | ExpMove ±3.0%（近端） | Rank 65%
量化视角： IV 中性（Rank 65%）｜期限结构倒挂（Term 0.85，近月 IV 高于远月）｜保护溢价薄（Skew 0.2pp）｜当日成交偏 Put（P/C量 1.22）——观察点，非方向信号
...
Gamma Regime: NEGATIVE（模型分类） | GEX(存量) -20,845,489 | GEX Change vs 上次快照 -849,208 | Flip: Primary Flip: 165.80（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 97%（带内） ｜ IV 有效性: VALID 392 / LOW 104 / INVALID 356
   ⇒ 全链负Gamma，波动易被放大（模型层）
结构观察区: Primary Flip 165.80（全链重定价，覆盖 97%）
Put Wall 158（弱结构｜现价高于该位 2.5%） | Call Wall 170（弱结构｜现价低于该位 4.7%）
最近结构参考: Flip 166（现价低于该位 2.3%）
量化视角： 负 Gamma（2085万，无历史分位）｜负 Gamma 加深（85万）｜现价位于 Flip 下方 2.34%｜⚠️ 重点观察：负 Gamma 且日内加深——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
