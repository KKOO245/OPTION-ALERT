...
Options: P/C成交量 0.56 | OI比 0.52 | ATM IV 53.8% | Skew 2.6pp | Term 0.86 | ExpMove ±4.0%（近端） | Rank 87%
量化视角： IV 历史高位（Rank 87%，期权偏贵）｜期限结构倒挂（Term 0.86，近月 IV 高于远月）｜保护溢价中性（Skew 2.6pp）｜存量 Call 偏重（OI比 0.52）——观察点，非方向信号
   ⇒ Put/Call Volume: 0.56×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.52×（存量 Call 仓位高于 Put）→ 存量 Call-dominant
   ⇒ 两者结构一致
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量Call-dominant
   ExpMove 期限化（expmove_v1）: 09-11（3D）±4.0% ｜ 09-18（10D）±6.3% ｜ 09-25（17D）±7.9% ｜ 10-02（24D）±9.8%
   ⇒ IV–VIX Spread: +38.1pp*（*近月 ATM IV − VIX；期限未对齐，仅作相对波动率 Proxy，不直接代表期权定价贵/便宜）
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 45,010,274 | GEX Change vs 上次快照 2,120,090 | Flip: Primary Flip: 97.05（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 96%（带内） ｜ IV 有效性: VALID 487 / LOW 154 / INVALID 309
结构观察区: Primary Flip 97.05（全链重定价，覆盖 96%）
Call Wall 100（弱结构｜现价低于该位 0.1%）
最近结构参考: Call Wall 100（现价低于该位 0.1%）
量化视角： 正 Gamma（4501万，无历史分位）｜正 Gamma 增强（+212万）｜现价位于 Flip 上方 2.92%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
📅 事件差分（观察，非因果）: 09-11（3D）ATM IV 53.8% vs 09-18 48.1%（差 +5.6pp）——覆盖 PPI 生产者物价 MoM、成屋销售、Core Inflation Rate MoM、Inflation Rate MoM、Inflation Rate YoY、Core Inflation Rate YoY、密歇根消费者信心 Consumer Sentiment Prel
