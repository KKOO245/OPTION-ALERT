...
🟡 **事件差分**: 09-04 ATM IV 42.1% vs 09-11 30.8%（差 +11.2pp）
...
Options: P/C成交量 0.76 | OI比 0.58 | ATM IV 42.1% | Skew 6.1pp | Term 0.74 | ExpMove ±2.4%（近端） | Rank — (历史不足)
量化视角： 期限结构倒挂（Term 0.74，近月 IV 高于远月）｜保护溢价显著（Skew 6.1pp，Put 明显贵于 Call）｜存量 Call 偏重（OI比 0.58）——观察点，非方向信号
   ⇒ Put/Call Volume: 0.76×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.58×（存量 Call 仓位高于 Put）→ 存量 Call-dominant
   ⇒ 两者结构一致
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量Call-dominant
   ExpMove 期限化（expmove_v1）: 09-04（1D）±2.4% ｜ 09-11（8D）±4.0% ｜ 09-18（15D）±5.7% ｜ 09-25（22D）±7.5%
   ⇒ IV–VIX Spread: +27.4pp*（*近月 ATM IV − VIX；期限未对齐，仅作相对波动率 Proxy，不直接代表期权定价贵/便宜）
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: NEGATIVE（模型分类） | GEX(存量) -1,290,941 | GEX Change vs 上次快照 -67,347 | Flip: Primary Flip: 374.40（PRIMARY，全链重定价 + 覆盖达标）
...
Call Wall 400（现价低于该位 7.8%）
最近结构参考: Flip 374（现价低于该位 1.5%）
量化视角： 负 Gamma（129万，无历史分位）｜负 Gamma 加深（7万）｜现价位于 Flip 下方 1.45%｜⚠️ 重点观察：负 Gamma 且日内加深——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
存量OI:      C 2.2k / P 1.2k
今日变化ΔOI: C +0.1k / P +29
平值价格ATM:  C 2.50 / P 6.20
隐含波动率 ATM IV:  42.1%
净 delta 敞口变化 ΔOI Δ Exposure*: -542 shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 390 ｜ +59 ｜ $0.10 ｜ 名义 $590* ｜ +5.7%
C 400 ｜ +31 ｜ $0.20 ｜ 名义 $620* ｜ +8.4%
C 380 ｜ +26 ｜ $0.10 ｜ 名义 $260* ｜ +3.0%
结构参考：390（+5.7%）附近形成 OI 变化集中（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
量化解读： 存量 Call 重｜ATM IV 42.1%｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 负 542 股（方向不可观测）——方向不可观测，观察点，非方向信号
...
📅 事件差分（观察，非因果）: 09-04（1D）ATM IV 42.1% vs 09-11 30.8%（差 +11.2pp）——覆盖 ISM 非制造业 PMI、Non Farm Payrolls、失业率
