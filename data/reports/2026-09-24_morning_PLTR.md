...
PLTR  昨收 191.79 → 今开 189.20（-1.4%） | 较昨收变动（含盘初走势） ｜ 今日高 192.76 ｜ 低 188.24

Options: P/C成交量 0.54 | OI比 0.72 | ATM IV 49.5% | Skew 1.5pp | Term 0.93 | ExpMove ±3.6%（近端） | Rank 31%
量化视角： IV 中性（Rank 31%）｜期限结构正常（Term 0.93）｜保护溢价薄（Skew 1.5pp）｜存量 Call 偏重（OI比 0.72）——观察点，非方向信号
   ⇒ Put/Call Volume: 0.54×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.72×（存量 Call 仓位高于 Put）→ 存量 Call-dominant
   ⇒ 两者结构一致
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量Call-dominant
   ExpMove 期限化（expmove_v1）: 09-25（1D）±3.6% ｜ 10-02（8D）±6.2% ｜ 10-09（15D）±7.9% ｜ 10-16（22D）±9.4%
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 99,714,653 | GEX Change vs 上次快照 4,361,459 | Flip: Primary Flip: 173.14（PRIMARY，全链重定价 + 覆盖达标）
...
量化视角： 正 Gamma（9971万，无历史分位）｜正 Gamma 增强（+436万）｜现价位于 Flip 上方 6.92%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
量化解读： 存量 Call 重｜ATM IV 49.5%｜历史 Rank 31%（近端代理）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 0 股
...
数据溯源：完整表见附录 / thesis / analytics/daily/2026-09-24/PLTR_morning.json