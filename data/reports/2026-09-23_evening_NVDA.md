...
Options: P/C成交量 0.34 | OI比 0.74 | ATM IV 34.0% | Skew 0.3pp | Term 0.93 | ExpMove ±2.8%（近端） | Rank 16%
量化视角： IV 历史低位（Rank 16%，期权偏便宜）｜期限结构正常（Term 0.93）｜保护溢价薄（Skew 0.3pp）｜存量 Call 偏重（OI比 0.74）——观察点，非方向信号
...
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 618,191,221 | GEX Change vs 上次快照 -164,167,277 | Flip: Primary Flip: 214.06（PRIMARY，全链重定价 + 覆盖达标）
...
Call Wall 230（现价低于该位 2.0%）
最近结构参考: Call Wall 230（现价低于该位 2.0%）
量化视角： 正 Gamma（6.18亿，无历史分位）｜正 Gamma 减弱（1.64亿）｜现价位于 Flip 上方 5.35%｜⚠️ 重点观察：正 Gamma 由正转负（结构切换）——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
量化解读： 存量 Call 重｜ATM IV 32.1%｜历史 Rank 16%（近端代理）｜IV/RV 1.05×（近似）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 0 股
