...
🟡 **单日价格波动**: +2.7%（vs 前收盘）
   ⇒ 价格变动超阈值；纯事实，不解释方向
🟡 **近现价集中开仓**: 09-18 99C ΔOI +10,398（距现价 +0.4%）
...
GDX  昨收 96.03 → 今开 98.21（+2.3%） | 较昨收变动（含盘初走势） ｜ 今日高 99.08 ｜ 低 97.42

Options: P/C成交量 1.37 | OI比 0.96 | ATM IV 69.0% | Skew 1.3pp | Term 0.66 | ExpMove ±5.5%（近端） | Rank 98%
量化视角： IV 历史高位（Rank 98%，期权偏贵）｜期限结构倒挂（Term 0.66，近月 IV 高于远月）｜保护溢价薄（Skew 1.3pp）｜当日成交偏 Put（P/C量 1.37）——观察点，非方向信号
   ⇒ Put/Call Volume: 1.37×（Put 成交量高于 Call）→ 方向 Unknown
   ⇒ Put/Call OI: 0.96×（两侧接近均衡）
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Put，存量接近均衡
   ExpMove 期限化（expmove_v1）: 09-18（7D）±5.5% ｜ 09-25（14D）±7.4% ｜ 10-02（21D）±10.4% ｜ 10-09（28D）±10.7%
   ⇒ IV–VIX Spread: +53.0pp*（*近月 ATM IV − VIX；期限未对齐，仅作相对波动率 Proxy，不直接代表期权定价贵/便宜）
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 16,340,551 | GEX Change vs 上次快照 43,940,711 | Flip: Primary Flip: 97.91（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 94%（带内） ｜ IV 有效性: VALID 499 / LOW 182 / INVALID 269
结构观察区: Primary Flip 97.91（全链重定价，覆盖 94%）
Call Wall 100（弱结构｜现价低于该位 1.4%）
最近结构参考: Flip 98（现价高于该位 0.7%）
量化视角： 正 Gamma（1634万，无历史分位）｜由负转正（+4394万）｜现价位于 Flip 上方 0.73%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
• 支撑/压力参考：下方 97（MaxPain，仅结算参考）；上方 100（Call Wall，弱结构）。
• Gamma 区域：切换参考 98（全链重定价，覆盖 94%）。
...
平值价格ATM:  C 2.53 / P 2.89
隐含波动率 ATM IV:  47.1%
净 delta 敞口变化 ΔOI Δ Exposure*: 572k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 99 ｜ +10,398 ｜ $2.53 ｜ 名义 $2.63M* ｜ +0.4%
C 103 ｜ +10,214 ｜ $1.05 ｜ 名义 $1.07M* ｜ +4.4%
P 91 ｜ +5,197 ｜ $0.42 ｜ 名义 $218.3k* ｜ -7.7%
结构参考：99（+0.4%） / 91（-7.7%）形成 OI 变化集中区（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
该期限仓位参考（Wall 同墙位口径，Max Pain 仅结算参考）: Max Pain 91（结算参考） ｜ Call Wall 100（+1.4%，弱）（OI 23.5k）
量化解读： 存量 Put 重｜⚠️ 背离：存量 Put 重但当日 Call 增仓更多｜ATM IV 47.1%｜历史 Rank 98%（近端代理）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 571,639 股（方向不可观测）——方向不可观测，观察点，非方向信号

📆 09-25 Forward Structure
存量OI:      C 7.1k / P 9.6k
今日变化ΔOI: C +87 / P +1.7k
平值价格ATM:  C 3.67 / P 3.60
隐含波动率 ATM IV:  46.7%
净 delta 敞口变化 ΔOI Δ Exposure*: -28k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
P 95 ｜ +352 ｜ $2.06 ｜ 名义 $72.5k* ｜ -3.7%
C 105 ｜ -317 ｜ $1.42 ｜ 名义 $-45.0k* ｜ +6.5%
（已过滤 1 条低相关性彩票：名义 <$50k 且距现价 >10%）
结构参考：95（-3.7%）附近形成 OI 变化集中（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
该期限仓位参考（Wall 同墙位口径，Max Pain 仅结算参考）: Max Pain 96（结算参考） ｜ Put Wall 90（-8.7%，弱）（OI 1.2k）
量化解读： 存量 Put 重｜ATM IV 46.7%｜历史 Rank 98%（近端代理）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 负 28,425 股（方向不可观测）——方向不可观测，观察点，非方向信号

10-02（MEDIUM △）Top ΔOI: 96P +130 ｜ 104C +58
10-02（MEDIUM △）仓位参考: Max Pain 99（结算参考） ｜ Put Wall 97（-1.7%）（OI 15.1k）
