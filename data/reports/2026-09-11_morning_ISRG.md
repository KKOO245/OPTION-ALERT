...
🔍 重点速览
🟡 **近现价集中开仓**: 09-18 362C ΔOI +72（距现价 -1.1%）
...
Options: P/C成交量 0.26 | OI比 0.73 | ATM IV 40.9% | Skew -1.1pp | Term 0.82 | ExpMove ±6.3%（近端） | Rank — (历史不足)
量化视角： 期限结构倒挂（Term 0.82，近月 IV 高于远月）｜Put 保护异常便宜（Skew -1.1pp，Put IV < Call IV）｜存量 Call 偏重（OI比 0.73）——观察点，非方向信号
   ⇒ Put/Call Volume: 0.26×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.73×（存量 Call 仓位高于 Put）→ 存量 Call-dominant
   ⇒ 两者结构一致
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量Call-dominant
   ExpMove 期限化（expmove_v1）: 09-18（7D）±6.3% ｜ 09-25（14D）±0.0% ｜ 10-02（21D）±6.6% ｜ 10-09（28D）±9.4%
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: NEGATIVE（模型分类） | GEX(存量) -674,063 | GEX Change vs 上次快照 1,686,089 | Flip: Primary Flip: 368.92（PRIMARY，全链重定价 + 覆盖达标）
...
Put Wall 350（弱结构｜现价高于该位 4.8%） | Call Wall 400（弱结构｜现价低于该位 8.3%）
最近结构参考: Flip 369（现价低于该位 0.6%）
量化视角： 负 Gamma（67万，无历史分位）｜负 Gamma 缓解（+169万）｜现价位于 Flip 下方 0.60%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
今日变化ΔOI: C +0.2k / P +43
平值价格ATM:  C 4.82 / P 18.40
隐含波动率 ATM IV:  35.7%
净 delta 敞口变化 ΔOI Δ Exposure*: 10k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 362 ｜ +72 ｜ $6.75 ｜ 名义 $48.6k* ｜ -1.1%
C 370 ｜ +58 ｜ $3.68 ｜ 名义 $21.3k* ｜ +0.9%
P 340 ｜ +42 ｜ $1.15 ｜ 名义 $4.8k* ｜ -7.3%
结构参考：370（+0.9%） / 362（-1.1%）形成 OI 变化集中区（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
该期限仓位参考（Wall 同墙位口径，Max Pain 仅结算参考）: Max Pain 372（结算参考） ｜ Call Wall 400（+9.1%，弱）（OI 1.1k）
量化解读： 存量两侧均衡｜⚠️ 背离：存量 Put 重但当日 Call 增仓更多｜ATM IV 35.7%｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 10,492 股（方向不可观测）——方向不可观测，观察点，非方向信号

09-25（MEDIUM △）Top ΔOI: 362C +22 ｜ 357C +20
09-25（MEDIUM △）仓位参考: Max Pain 365（结算参考） ｜ Put Wall 360（-1.8%，弱）（OI 0.2k）

10-02（MEDIUM △）Top ΔOI: 370C +23 ｜ 390C +9
10-02（MEDIUM △）仓位参考: Max Pain 365（结算参考） ｜ Put Wall 335（-8.6%）（OI 2.4k）

10-09（Activity LOW）仓位参考: Max Pain 350（结算参考） ｜ Call Wall 375（+2.3%，弱）（OI 54）
