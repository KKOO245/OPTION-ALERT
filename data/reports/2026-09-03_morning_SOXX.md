...
🔍 重点速览
🟡 **近现价集中开仓**: 09-04 475P ΔOI +1,050（距现价 -4.7%）
...
SOXX  昨收 501.44 → 今开 496.34（-1.0%） | 较昨收变动（含盘初走势） ｜ 今日高 499.57 ｜ 低 489.21

Options: P/C成交量 0.46 | OI比 0.89 | ATM IV 38.6% | Skew 6.8pp | Term 0.94 | ExpMove ±2.0%（近端） | Rank 66%
量化视角： IV 中性（Rank 66%）｜期限结构正常（Term 0.94）｜保护溢价显著（Skew 6.8pp，Put 明显贵于 Call）｜当日成交与存量接近均衡——观察点，非方向信号
   ⇒ Put/Call Volume: 0.46×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.89×（两侧接近均衡）
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量接近均衡
   ExpMove 期限化（expmove_v1）: 09-04（1D）±2.0% ｜ 09-11（8D）±4.0% ｜ 09-18（15D）±5.3% ｜ 09-25（22D）±10.7%
   ⇒ IV–VIX Spread: +23.7pp*（*近月 ATM IV − VIX；期限未对齐，仅作相对波动率 Proxy，不直接代表期权定价贵/便宜）
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: NEGATIVE（模型分类） | GEX(存量) -29,854,550 | GEX Change vs 上次快照 -5,508,683 | Flip: Primary Flip: 514.65（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 91%（带内） ｜ IV 有效性: VALID 496 / LOW 360 / INVALID 706
   ⇒ 全链负Gamma，波动易被放大（模型层）
结构观察区: Primary Flip 514.65（全链重定价，覆盖 91%）
Put Wall 450（弱结构｜现价高于该位 10.7%）
最近结构参考: Flip 515（现价低于该位 3.2%）
量化视角： 负 Gamma（2985万，无历史分位）｜负 Gamma 加深（551万）｜现价位于 Flip 下方 3.19%｜⚠️ 重点观察：负 Gamma 且日内加深——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
平值价格ATM:  C 3.40 / P 6.80
隐含波动率 ATM IV:  38.6%
净 delta 敞口变化 ΔOI Δ Exposure*: 22k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
P 475 ｜ +1,050 ｜ $0.40 ｜ 名义 $42.0k* ｜ -4.7%
P 467 ｜ +853 ｜ $0.25 ｜ 名义 $21.3k* ｜ -6.2%
（已过滤 1 条低相关性彩票：名义 <$50k 且距现价 >10%）
结构参考：475（-4.7%）附近形成 OI 变化集中（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
量化解读： 存量两侧均衡｜⚠️ 背离：存量 Call 重但当日 Put 增仓更多｜ATM IV 38.6%｜历史 Rank 66%（近端代理）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 22,130 股（方向不可观测）——方向不可观测，观察点，非方向信号
...
📅 事件差分（观察，非因果）: 09-04（1D）ATM IV 38.6% vs 09-11 32.4%（差 +6.2pp）——覆盖 ISM 非制造业 PMI、Non Farm Payrolls、失业率
