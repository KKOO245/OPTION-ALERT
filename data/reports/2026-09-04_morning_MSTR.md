...
Options: P/C成交量 0.65 | OI比 1.04 | ATM IV 115.2% | Skew -5.7pp | Term 0.63 | ExpMove ±8.1%（近端） | Rank 88%
量化视角： IV 历史高位（Rank 88%，期权偏贵）｜期限结构倒挂（Term 0.63，近月 IV 高于远月）｜Put 保护异常便宜（Skew -5.7pp，Put IV < Call IV）｜当日成交与存量接近均衡——观察点，非方向信号
   ⇒ Put/Call Volume: 0.65×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 1.04×（两侧接近均衡）
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量接近均衡
   ExpMove 期限化（expmove_v1）: 09-11（7D）±8.1% ｜ 09-18（14D）±11.3% ｜ 09-25（21D）±13.6% ｜ 10-02（28D）±16.5%
   ⇒ IV–VIX Spread: +101.1pp*（*近月 ATM IV − VIX；期限未对齐，仅作相对波动率 Proxy，不直接代表期权定价贵/便宜）
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 90,484,610 | GEX Change vs 上次快照 -6,128,799 | Flip: Primary Flip: 122.11（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 96%（带内） ｜ IV 有效性: VALID 862 / LOW 143 / INVALID 327
结构观察区: Primary Flip 122.11（全链重定价，覆盖 96%）
最近结构参考: Flip 122（现价高于该位 15.1%）
量化视角： 正 Gamma（9048万，无历史分位）｜正 Gamma 减弱（613万）｜现价位于 Flip 上方 15.13%｜⚠️ 重点观察：正 Gamma 由正转负（结构切换）——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
• Gamma 区域：切换参考 122（全链重定价，覆盖 96%）。
...
平值价格ATM:  C 5.63 / P 5.75
隐含波动率 ATM IV:  71.0%
净 delta 敞口变化 ΔOI Δ Exposure*: 3.4M shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 145 ｜ +15,957 ｜ $4.05 ｜ 名义 $6.46M* ｜ +3.1%
C 152 ｜ +13,754 ｜ $2.22 ｜ 名义 $3.05M* ｜ +8.5%
C 147 ｜ +13,563 ｜ $3.32 ｜ 名义 $4.50M* ｜ +4.6%
结构参考：145（+3.1%）附近形成 OI 变化集中（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
量化解读： 存量 Call 重｜ATM IV 71.0%｜历史 Rank 88%（近端代理）｜净 delta 敞口 正 3,361,651 股（方向不可观测）——方向不可观测，观察点，非方向信号

📆 09-18 Forward Structure
存量OI:      C 266.1k / P 193.5k
今日变化ΔOI: C -3.2k / P +5.7k
平值价格ATM:  C 8.40 / P 7.53
隐含波动率 ATM IV:  72.2%
净 delta 敞口变化 ΔOI Δ Exposure*: -278k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 160 ｜ -787 ｜ $2.80 ｜ 名义 $-220.4k* ｜ +13.8%
（已过滤 2 条低相关性彩票：名义 <$50k 且距现价 >10%）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
量化解读： 存量 Call 重｜⚠️ 背离：存量 Call 重但当日 Put 增仓更多｜ATM IV 72.2%｜历史 Rank 88%（近端代理）｜净 delta 敞口 负 277,657 股（方向不可观测）——方向不可观测，观察点，非方向信号
...
平值价格ATM:  C 11.37 / P 11.81
隐含波动率 ATM IV:  72.7%
净 delta 敞口变化 ΔOI Δ Exposure*: 19k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 180 ｜ +1,967 ｜ $2.50 ｜ 名义 $491.8k* ｜ +28.0%
C 230 ｜ +1,060 ｜ $0.59 ｜ 名义 $62.5k* ｜ +63.6%
（已过滤 1 条低相关性彩票：名义 <$50k 且距现价 >10%）
结构参考：180（+28.0%）附近形成 OI 变化集中（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
量化解读： 存量 Put 重｜⚠️ 背离：存量 Put 重但当日 Call 增仓更多｜ATM IV 72.7%｜历史 Rank 88%（近端代理）｜净 delta 敞口 正 19,130 股（方向不可观测）——方向不可观测，观察点，非方向信号
