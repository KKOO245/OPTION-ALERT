...
🔍 重点速览
🟡 **事件差分**: 09-04 ATM IV 79.2% vs 09-11 64.7%（差 +14.5pp）
   ⇒ 覆盖事件的期限隐含波动相对相邻期限偏高（观察，非因果）
🟡 **近现价集中开仓**: 09-04 54C ΔOI +131（距现价 -1.1%）
...
Options: P/C成交量 0.32 | OI比 0.75 | ATM IV 79.2% | Skew 2.7pp | Term 0.79 | ExpMove ±5.0%（近端） | Rank 68%
量化视角： IV 中性（Rank 68%）｜期限结构倒挂（Term 0.79，近月 IV 高于远月）｜保护溢价中性（Skew 2.7pp）｜存量 Call 偏重（OI比 0.75）——观察点，非方向信号
   ⇒ Put/Call Volume: 0.32×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.75×（存量 Call 仓位高于 Put）→ 存量 Call-dominant
   ⇒ 两者结构一致
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量Call-dominant
   ExpMove 期限化（expmove_v1）: 09-04（2D）±5.0% ｜ 09-11（9D）±9.3% ｜ 09-18（16D）±10.8% ｜ 09-25（23D）±13.3%
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: NEGATIVE（模型分类） | GEX(存量) -660,108 | GEX Change vs 上次快照 1,242,895 | Flip: Primary Flip: 54.94（PRIMARY，全链重定价 + 覆盖达标）
...
Put Wall 55（弱结构｜现价低于该位 0.8%） | Call Wall 60（弱结构｜现价低于该位 9.0%）
最近结构参考: Flip 55（现价低于该位 0.6%）
量化视角： 负 Gamma（66万，无历史分位）｜负 Gamma 缓解（+124万）｜现价位于 Flip 下方 0.64%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
平值价格ATM:  C 1.23 / P 1.50
隐含波动率 ATM IV:  79.2%
净 delta 敞口变化 ΔOI Δ Exposure*: 28k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 57 ｜ +169 ｜ $0.54 ｜ 名义 $9.1k* ｜ +4.4%
P 50 ｜ +152 ｜ $0.14 ｜ 名义 $2.1k* ｜ -8.4%
C 54 ｜ +131 ｜ $1.67 ｜ 名义 $21.9k* ｜ -1.1%
结构参考：57（+4.4%） / 50（-8.4%）形成 OI 变化集中区（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
量化解读： 存量 Call 重｜ATM IV 79.2%｜历史 Rank 68%（近端代理）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 28,380 股（方向不可观测）——方向不可观测，观察点，非方向信号

📆 09-11 Forward Structure
存量OI:      C 3.6k / P 3.9k
今日变化ΔOI: C +0.2k / P +0.2k
平值价格ATM:  C 2.06 / P 3.00
隐含波动率 ATM IV:  64.7%
净 delta 敞口变化 ΔOI Δ Exposure*: 5k shares
Top ΔOI（行权价 ｜ ΔOI ｜ 最新价 ｜ 名义金额* ｜ 距现价）:
C 60 ｜ +65 ｜ $0.56 ｜ 名义 $3.6k* ｜ +9.9%
（已过滤 2 条低相关性彩票：名义 <$50k 且距现价 >10%）
结构参考：60（+9.9%）附近形成 OI 变化集中（结构观察，非价格预测）
*模型估算/名义金额代理；买开/卖开方向不可观测（Scenario A/B）
量化解读： 存量两侧均衡｜ATM IV 64.7%｜历史 Rank 68%（近端代理）｜期限正常（远月高于近端）｜净 delta 敞口 正 4,692 股（方向不可观测）——方向不可观测，观察点，非方向信号

09-18（MEDIUM △）Top ΔOI: 60P -265 ｜ 55C +110

📅 事件差分（观察，非因果）: 09-04（2D）ATM IV 79.2% vs 09-11 64.7%（差 +14.5pp）——覆盖 ISM 非制造业 PMI、Non Farm Payrolls、失业率
