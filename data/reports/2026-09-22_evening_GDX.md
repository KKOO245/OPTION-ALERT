...
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 82,210,019 | GEX Change vs 上次快照 38,963,029 | Flip: Primary Flip: 93.45（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 99%（带内） ｜ IV 有效性: VALID 488 / LOW 99 / INVALID 275
结构观察区: Primary Flip 93.45（全链重定价，覆盖 99%）
Call Wall 100（现价低于该位 2.2%）
最近结构参考: Call Wall 100（现价低于该位 2.2%）
量化视角： 正 Gamma（8221万，无历史分位）｜正 Gamma 增强（+3896万）｜现价位于 Flip 上方 4.69%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
量化解读： 存量 Call 重｜ATM IV 46.4%｜历史 Rank 75%（近端代理）｜IV/RV 1.29×（近似）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 0 股
