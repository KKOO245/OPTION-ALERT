...
SLV  昨收 59.82 → 今开 59.16（-1.1%） | 较昨收变动（含盘初走势） ｜ 今日高 59.97 ｜ 低 59.13
...
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 102,408,178 | GEX Change vs 上次快照 5,030,698 | Flip: Primary Flip: 56.47（PRIMARY，全链重定价 + 覆盖达标）
...
量化视角： 正 Gamma（1.02亿，无历史分位）｜正 Gamma 增强（+503万）｜现价位于 Flip 上方 5.93%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
数据溯源：完整表见附录 / thesis / analytics/daily/2026-09-07/SLV_morning.json