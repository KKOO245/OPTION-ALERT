...
NNE: 今开 17.43 → 收盘 17.59（+0.9%） ｜ 今日高 17.76 ｜ 低 17.17
Target 状态: 无待验证 Target（今日无 Setup 触发）

Options: P/C成交量 0.27 | OI比 0.69 | ATM IV 86.5% | Skew 6.2pp | Term 0.88 | ExpMove ±5.7%（近端） | Rank 12%
量化视角： IV 历史低位（Rank 12%，期权偏便宜）｜期限结构倒挂（Term 0.88，近月 IV 高于远月）｜保护溢价显著（Skew 6.2pp，Put 明显贵于 Call）｜存量 Call 偏重（OI比 0.69）——观察点，非方向信号
   ...
   ExpMove 期限化（expmove_v1）: 09-04（2D）±5.7% ｜ 09-11（9D）±8.8% ｜ 09-18（16D）±13.1% ｜ 09-25（23D）±13.8%
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 595,739 | GEX Change vs 上次快照 404,811 | Flip: Primary Flip: 17.11（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 92%（带内） ｜ IV 有效性: VALID 220 / LOW 87 / INVALID 159
结构观察区: Primary Flip 17.11（全链重定价，覆盖 92%）
Put Wall 16（弱结构｜现价高于该位 9.9%）
最近结构参考: Flip 17（现价高于该位 2.8%）
量化视角： 正 Gamma（60万，无历史分位）｜正 Gamma 增强（+40万）｜现价位于 Flip 上方 2.81%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
• 支撑/压力参考：下方 16（Put Wall，弱结构）；上方 18（MaxPain，仅结算参考）。
• Gamma 区域：切换参考 17（全链重定价，覆盖 92%）。
• 做市商（条件机制）：若 Scenario A + 负 Gamma 成立，跌破关键位下方可能对应顺周期卖出压力增加；实际做市商对冲流量不可观测。Scenario B → 方向相反。不进入方向决策。
• 失效参考：跌破关键位结构参考失效（结构性参考，非预测）。
🔺 Activity（事实层，方向 Unknown）
09-25 15.0P — Vol 1（Yahoo补） | 最新价 $0.37 | OI 27→197 (ΔOI +170张) | ΔOI/Volume N/A（量数据不完整） | Magnitude: MEDIUM | 完整度: HIGH
   ⇒ 放量且净增170张（+629.6% vs前日OI），值得跟踪（方向未知）
...
10-02 17.0P — Vol 1（Yahoo补） | 最新价 $1.26 | OI 14→123 (ΔOI +109张) | ΔOI/Volume N/A（量数据不完整） | Magnitude: MEDIUM | 完整度: HIGH
   ⇒ 放量且净增109张（+778.6% vs前日OI），值得跟踪（方向未知）
...
09-25  C +36 / P +0.2k ｜ Activity MEDIUM △ ｜ 23D
...
P 17 ｜ +158 ｜ $0.25 ｜ 名义 $4.0k* ｜ -3.4%
C 18 ｜ +59 ｜ $0.20 ｜ 名义 $1.2k* ｜ +5.2%
C 17 ｜ +51 ｜ $0.55 ｜ 名义 $2.8k* ｜ -0.5%
结构参考：18（+5.2%） / 17（-3.4%）形成 OI 变化集中区（结构观察，非价格预测）
...
数据溯源：完整表见附录 / thesis / analytics/daily/2026-09-02/NNE_evening.json