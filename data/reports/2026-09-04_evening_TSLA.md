...
TSLA: 今开 362.18 → 收盘 354.08（-2.2%） ｜ 今日高 364.69 ｜ 低 351.32 ｜ 昨收 376.36 → 收盘 354.08（-5.9%）
...
🧭 结构解读（全部依赖上方假设）
• 支撑/压力参考：上方 360（Put Wall，弱结构） / 362（MaxPain，仅结算参考）。
...
该期限仓位参考（Wall 同墙位口径，Max Pain 仅结算参考）: Max Pain 365（结算参考） ｜ Put Wall 360（+1.7%）（OI 13.2k）
量化解读： 存量 Call 重｜ATM IV 32.0%｜历史 Rank 3%（近端代理）｜期限正常（远月高于近端）｜净 delta 敞口 正 0 股（方向不可观测）——方向不可观测，观察点，非方向信号

09-11（Activity LOW）仓位参考: Max Pain 360（结算参考）

09-14（Activity LOW）仓位参考: Max Pain 370（结算参考） ｜ Put Wall 370（+4.5%，弱）（OI 0.8k）

09-16（Activity LOW）仓位参考: Max Pain 365（结算参考） ｜ Put Wall 360（+1.7%）（OI 0.5k）
