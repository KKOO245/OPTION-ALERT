...
NBIS  昨收 236.12 → 今开 235.16（-0.4%） | 较昨收变动（含盘初走势） ｜ 今日高 236.57 ｜ 低 228.50

Options: P/C成交量 0.52 | OI比 0.86 | ATM IV 87.9% | Skew -2.6pp | Term 0.92 | ExpMove ±6.4%（近端） | Rank 21%
量化视角： IV 历史低位（Rank 21%，期权偏便宜）｜期限结构正常（Term 0.92）｜Put 保护异常便宜（Skew -2.6pp，Put IV < Call IV）｜当日成交与存量接近均衡——观察点，非方向信号
   ⇒ Put/Call Volume: 0.52×（Call 成交量高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.86×（两侧接近均衡）
   ⇒ 当日成交 vs 存量仓位：当日成交偏 Call，存量接近均衡
   ExpMove 期限化（expmove_v1）: 09-25（2D）±6.4% ｜ 10-02（9D）±10.6% ｜ 10-09（16D）±14.1% ｜ 10-16（23D）±16.5%
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 16,110,393 | GEX Change vs 上次快照 263,638 | Flip: Primary Flip: 219.12（PRIMARY，全链重定价 + 覆盖达标）
...
最近结构参考: Flip 219（现价高于该位 7.9%）
量化视角： 正 Gamma（1611万，无历史分位）｜正 Gamma 增强（+26万）｜现价位于 Flip 上方 7.93%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
量化解读： 存量两侧均衡｜ATM IV 87.9%｜历史 Rank 21%（近端代理）｜IV/RV 1.53×（近似）｜期限倒挂（近端 IV > 远月）｜净 delta 敞口 正 0 股
...
📅 事件差分（观察，非因果）: 09-25（2D）ATM IV 87.9% vs 10-02 81.4%（差 +6.5pp）——覆盖 President Trump and President Xi Summit、耐用品订单 Orders MoM
   符合'覆盖事件的期权溢价更高'（美联储 IFDP 1376 实证；单日截面，需连续多日确认）

数据质量: 行情 A ｜ 期权结构 A ｜ 流向 C ｜ 做市商机制 C —— Flow 相关层（Activity 连续性、做市商机制解读）置信度受限。
Setup: 今日无 Setup 触发（机械检查全部 Setup）

数据溯源：完整表见附录 / thesis / analytics/daily/2026-09-23/NBIS_morning.json