...
量化视角： IV 中性（Rank 51%）｜期限结构正常（Term 1.13）｜Put 保护异常便宜（Skew -0.8pp，Put IV < Call IV）｜当日成交与存量接近均衡——观察点，非方向信号
   ...
   ExpMove 期限化（expmove_v1）: 09-25（3D）±2.5% ｜ 10-02（10D）±2.4% ｜ 10-09（17D）±8.5% ｜ 10-16（24D）±7.9%
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma Regime: POSITIVE（模型分类） | GEX(存量) 14,721,532 | GEX Change vs 上次快照 6,125,453 | Flip: Primary Flip: 546.83（PRIMARY，全链重定价 + 覆盖达标）
🔎 测量完整性: GEX 符号契约 gex_sign_v1（Model A: Call+ / Put−）｜ Gamma 口径 全链重定价 ｜ Effective GEX 覆盖: 98%（带内） ｜ IV 有效性: VALID 564 / LOW 266 / INVALID 676
结构观察区: Primary Flip 546.83（全链重定价，覆盖 98%）
Call Wall 600（现价低于该位 4.5%）
最近结构参考: Call Wall 600（现价低于该位 4.5%）
量化视角： 正 Gamma（1472万，无历史分位）｜正 Gamma 增强（+613万）｜现价位于 Flip 上方 4.75%——观察点，非方向信号
🧭 结构解读（全部依赖上方假设）
...
量化解读： 存量 Put 重｜ATM IV 33.5%｜历史 Rank 51%（近端代理）｜IV/RV 0.88×（近似）｜期限正常（远月高于近端）｜净 delta 敞口 正 0 股

10-02（Activity LOW）仓位参考: Max Pain 520 ｜ Call Wall 542.5（-5.3%，弱）（OI 2.8k）

10-09（Activity LOW）仓位参考: Max Pain 518 ｜ Call Wall 537.5（-6.2%，弱）（OI 0.4k）

10-16（Activity LOW）仓位参考: Max Pain 525 ｜ Call Wall 600（+4.8%）（OI 13.6k）
