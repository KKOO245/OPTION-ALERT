...
NVDA: 今晨 210.01 → 收盘 209.59（-0.2%）
Target 状态: PENDING（evaluation date …）——窗口结束前禁止'预测正确'类措辞


## NVDA

Options: P/C量 0.63 | OI比 0.64 | ATM IV 31.6% | Skew 0.3pp | Term 1.29 | ExpMove ±0.5% | Rank — (历史不足)
   ⇒ Put/Call Volume: 0.63×（Call 成交高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.64×（Put OI 低于 Call OI）→ 存量 Call-dominant
   ⇒ 两者结构一致
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma: POSITIVE | GEX(存量) N/A | GEX Change N/A | Flip: ≈237.13 / ≈269.05
结构观察区: 237–269（局部 Gamma 切换，低置信；Top-3 近似，需全链重定价验证）
距 Put Wall 190: +10.3% | 距 Call Wall 240: -12.7%
🧭 结构解读（全部依赖上方假设）
• 支撑/压力参考：下方 190（Put Wall）；上方 240（Call Wall）。
• Gamma 区域：切换参考 237（Top-3 近似，需全链重定价验证）。
...
数据溯源：完整表见附录 / thesis / analytics/daily/2026-08-24/NVDA_evening.json