...
Options: P/C量 0.63 | OI比 0.89 | ATM IV 90.3% | Skew -0.1pp | Term 0.89 | ExpMove ±7.7% | Rank — (历史不足)
   ⇒ Put/Call Volume: 0.63×（Call 成交高于 Put）→ 方向 Unknown
   ⇒ Put/Call OI: 0.89×（Put OI 低于 Call OI）→ 存量 Call-dominant
   ⇒ 两者结构一致
🔧 结构（未验证研究层：Mechanism Scenario A/B——OI 开仓方向不可观测）
Gamma: NEGATIVE | GEX(存量) N/A | GEX Change N/A | Flip: ≈1908.53
   ⇒ 全链负Gamma，波动易被放大（模型层）
结构观察区: ≈1909（局部 Gamma 切换，低置信；Top-3 近似，需全链重定价验证）
距 Put Wall 800: +89.3% | 距 Call Wall 2,000: -24.3%
🧭 结构解读（全部依赖上方假设）
• 支撑/压力参考：下方 800（Put Wall）；上方 2,000（Call Wall）。
• Gamma 区域：切换参考 1909（Top-3 近似，需全链重定价验证）。
